...
def collect_links(id_name,cookie=None):
    if not cookie:
        cookie = ''
    def ten_links(url,nnn,header=None):
    ...
    try:
        ...
        if  '验证' in str(soup):
            ...
            try:
                ...
                #等待
                time.sleep(9+random.randint(2,5))
                ten_one = ten_links(one_link,3)
                yes_title = ten_one[0]
                yes_title_url = 'https://mp.weixin.qq.com' + ten_one[1]
                print(str(id_name)+" 链接是："+str(one_link)+'\n' + yes_title)
                # print(ten_one)
                # print(yes_title)
                write_thing = '【微信名】：' + one_title +'\n' + '【微信id】：' + wi_id + '\n' + str(yes_title) + '\n' + str(yes_title_url) + '\n'+str(one_link)
                last_f.write(write_thing)
                last_f.close()
