...
def collect_links(id_name,page,cookie=None):
    ...
    for i in range(1,int(page)+1):
        print(f'第{str(page_n)}页--------------{id_name}')
        page_n = page_n+1
        url = f'https://weixin.sogou.com/weixin?query={id_name}&s_from=input&type=1&page={str(i)}&ie=utf8'
        ...
        header = {
            ...
            'Cookie': f'{cookie}',
            'Host': 'weixin.qq.com',
            'Referer': f'http://weixin.sogou.com/weixin?type=1&s_from=input&query={quote(id_name)}',
...
with open('aast.txt','w') as lastl_txt:
