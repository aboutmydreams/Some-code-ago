...
#书名以及TP
    ...
    while nnu <= (len(names_TPS_L)-1):
        ms_book = [names_TPS_L[nnu][0], names_TPS_L[nnu][1], z_L[nnu][0], z_L[nnu][1]]
...
def getrecommend():
    ...
    for x in content:
        ...
        a = [y.get_text() for y in recommend_list]
...
def getnowbook():
    ...
    while ln < len(hisbooks):
        newl = [hisbooks[ln].get_text(), tag_list[lm]]
