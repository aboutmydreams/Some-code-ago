...
def what_url(page_i):
    if page_i == 1:
        ...
        return url
    if page_i > 1:
        url = 'https://www.whitehouse.gov/news/page/{}/'.format(page_i)
        return url
...
def page_ten_link(i):
    ...
    page = s.get(url).text
    soup = BeautifulSoup(page,'lxml')
    f = open('link/'+str(i)+'.txt','w')
    try:
        for i in range(1,11):
            links = soup.select('#main-content > div.page-results > div > article:nth-of-type({}) > div > h2 > a'.format(str(i)))[0].get("href")
            f.write(links+'\n')
            print(str(i)+'成功----------')
            f.close()
    except IndexError:
...
def get_html_and_txt(url,i):
    ...
    a_dict["TYPE"]=type0
    a_dict["Title"] = htmlname.replace('-',' ')
    a_dict["Title_Url"] = url
    a_dict["Date"] = date
    a_dict["Author"] = author

    # -----------------------
    html_file=htmlname+".html"
    html_path=os.path.join(HTML_DIR,html_file)
    with open(html_path,"w",encoding='utf-8') as f:
        f.write(content)
        f.close()
    f1 = open('content_txt/{}.txt'.format(str(i)),'w')
    f1.write(str(a_dict))
    f1.close()


for i in range(0,4287):
    url = lines[i]
